...
| Provider scope        | Formatting is selected through an adapter `promptMode` capability. KoboldCPP, NovelAI raw models (such as Erato and Kayra), and registered text-completion plugins declare `text-completion`; chat-completion adapters use structured-message prompts. | Applying text wrappers to chat-completion APIs would be incorrect and can double-format prompts. Text-completion adapters declare the capability without hardcoded provider-ID checks in the prompt compiler. |
