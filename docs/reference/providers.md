...
- **Instruct Templates (Beta)**: Automatically format prompts locally using predefined templates (`auto`, `llama3`, `chatml`, `mistral`, `gemma2`, `alpaca`, `deepseek-r1`) or custom templates from **Options > Instruct Templates (Beta)**. See [Formatting & Instruct Templates](../guides/formatting-and-instruct.md).
