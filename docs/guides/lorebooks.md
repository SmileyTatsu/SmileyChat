...
SmileyChat supports importing native SmileyChat LoreBook JSON and SillyTavern World Info JSON files directly via drag-and-drop or the import dialog. You can also sync all World Info books automatically from a local SillyTavern installation using **Options > SillyTavern Sync**.
